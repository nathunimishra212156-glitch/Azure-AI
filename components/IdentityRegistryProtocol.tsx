...
const IdentityRegistryProtocol: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center p-10 animate-astral overflow-y-auto">
      <div className="w-full max-w-2xl space-y-12">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-500 text-sm font-bold uppercase tracking-[0.4em]">High Architect Oversight Required</p>
        </div>

        <div className="glass-panel rounded-[3rem] p-12 border-white/10 shadow-2xl relative overflow-hidden">
          ...
          <form onSubmit={handleRegister} className="space-y-8 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full bg-black/40 border border-white/5 rounded-2xl p-5 text-sm text-white focus:border-indigo-500/50 outline-none transition-all placeholder:text-slate-800"
                  placeholder="NEW_ARCHITECT_ID"
                />
              </div>
              <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                {roles.map(r => (
                  <button
                    ...
                    className={`px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all ${formData.role === r ? 'bg-indigo-600 border-indigo-400 text-white shadow-lg' : 'bg-white/5 border-white/5 text-slate-500 hover:bg-white/10'}`}
                  >
                    {r}
                  </button>
                ))}
            ...
            <button 
              type="submit"
              className="w-full py-6 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-[1.5rem] text-[12px] font-black uppercase tracking-[0.4em] text-white hover:opacity-90 transition-all shadow-xl active:scale-[0.98]"
            >
              Provision Authorisation Account
            </button>
          </form>
        </div>
        
        <div className="text-center">
          <p className="text-[9px] text-slate-700 font-bold uppercase tracking-[0.6em]">Nexus Core Identity Protocol v1.4.2</p>
        </div>
      </div>
    </div>
  );
